...
export function dotFor(
  ...
  if (phase === 'revising' || phase === 'review' || phase === 'approved')
    return 'accent';
...
export function eyebrowRight(
  ...
  switch (phase) {
    case 'approved':
      return { text: 'SETTLED', tone: 'accent' };
    case 'review':
      return { text: 'NEEDS YOUR APPROVAL', tone: 'accent' };
    case 'revising':
      return { text: 'WITH THE AGENT', tone: 'accent' };
    case 'cannot':
      return { text: 'UNCHANGED', tone: 'coral' };
    default:
      return pending + edits > 0
        ? { text: 'DRAFT · MARKED UP', tone: 'coral' }
...
export function phase(this: AppShell): Phase {
  return phaseOf({
    sealed: this.sealed,
    running: this.reviseRunning,
    waiting: this.reviseWaiting,
    handoff: this.handoff,
    cannot: this.seenCannot,
    landed: this.arrival ? this.arrival.n : 0,
    pendingCount: this.pendingCount,
    edits: this.changes.length,
  });
}
