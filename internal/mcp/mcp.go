// Package mcp is a minimal MCP server speaking newline-delimited JSON-RPC 2.0
// over a reader/writer pair — enough to be a Claude Code channel and expose a
// handful of tools, and nothing more.
//
// DELIBERATELY NOT THE SDK. Every shipped channel plugin is Bun +
// @modelcontextprotocol/sdk, and the spike for the channel design proved that
// is convention, not requirement: this framing, this handshake, and the
// experimental capability were accepted by Claude Code 2.1.226 from a
// dependency-free Go binary. Keeping it stdlib keeps galley one binary.
//
...
// because Notify is called from watcher goroutines while Run's loop is
...
// New starts the notification emitter alongside the Server. It is one
// goroutine for the Server's lifetime: Notify cannot write in the caller's
// goroutine, because a stdio peer reads at its own pace and a synchronous
// write would park a watcher on the client's schedule. Draining through one
// channel is also what keeps notifications in the order they were made.
...
// a bad key is refused here where the caller can see it. Callers start
// concurrently with Run, so Notify waits for Run to attach the writer before
// handing the notification to the emitter; a nil error means it is queued in
// order, not that the client has read it.
...
func (s *Server) Run(r io.Reader, w io.Writer) error {
	...
	select {
	case <-s.ready: // a second Run reuses the already-open latch
	default:
		close(s.ready)
	}
	s.mu.Unlock()
...
func (s *Server) answer(msg rpcMsg) error {
	...
	case "initialize":
		...
		if p.ProtocolVersion == "" {
			// Echoing the client's version is the whole policy; this is only
			// the fallback for a client that sent none.
