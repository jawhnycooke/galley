...
func TestInitializeDeclaresTheChannelCapability(t *testing.T) {
	w, sc := drive(t, Handler{Name: "galley", Version: "0.0.1", Instructions: "hello"})
...
func TestToolsListAndCall(t *testing.T) {
	called := ""
	h := Handler{
		Name: "galley", Version: "0.0.1",
		Tools: []Tool{{Name: "galley_ack", Description: "d", InputSchema: json.RawMessage(`{"type":"object"}`)}},
	...
	m := read(t, sc)
	if !strings.Contains(sc.Text(), "galley_ack") {
		t.Errorf("tools/list did not name the tool: %v", m)
	}
	send(t, w, `{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"galley_ack","arguments":{"doc":"x"}}}`)
	m = read(t, sc)
	if called != "galley_ack" {
		t.Error("Call was not dispatched")
	}
	if !strings.Contains(sc.Text(), "acknowledged") {
...
func TestNotificationsIgnoredUnknownMethodsRefused(t *testing.T) {
	w, sc := drive(t, Handler{Name: "g", Version: "1"})
...
func TestNotifyEmitsChannelNotificationAndValidatesMetaKeys(t *testing.T) {
	clientOut, serverIn := io.Pipe()
	serverOut, clientIn := io.Pipe()
	s := New(Handler{Name: "g", Version: "1"})
	go func() { _ = s.Run(clientOut, clientIn) }()
	defer func() { _ = serverIn.Close() }()
	sc := bufio.NewScanner(serverOut)

	if err := s.Notify("hello", map[string]string{"bad-key": "x"}); err == nil {
		t.Fatal("a hyphenated meta key must be rejected at construction — the client drops it silently")
	}
	if err := s.Notify("hello", map[string]string{"doc": "a.md", "reason": "revise"}); err != nil {
		t.Fatal(err)
	}
	if !sc.Scan() {
